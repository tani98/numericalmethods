 ...
 * @author Tani Aguirre
 */
public class ResolveFalseRuleFXMLController implements Initializable {
    

    FalseRule falseRule = new FalseRule();
        ...
        try {
            if (!isEmpty()) {
                if (isCorrect()) {
                    ...
                    falseRule.setFuntion(fx);
                    falseRule.setError(0.0001);
                    falseRule.setIterations(30);
                    falseRule.setRange(1.5, 2.5);
                    falseRule.solution();

                    listIteration.setItems(falseRule.getListSolution());
